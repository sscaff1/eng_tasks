 ...
 */
	public function add() {
		if ($this->request->is('post')) {
			$this->Role->create();
			$this->request->data['Role']['active'] = 1;
			if ($this->Role->save($this->request->data)) {
				$this->Session->setFlash(__('The job functon has been saved.'));
				return $this->redirect(array('controller' => 'tasks', 'action' => 'index'));
			} else {
				$this->Session->setFlash(__('The job function could not be saved. Please, try again.'));
			}
		}
	}
